...
For the Vercel frontend, set `API_URL` to `https://shirishtisnetflix.onrender.com` (without a trailing slash). Vite is configured to expose this public value to the browser as `import.meta.env.API_URL`. A local `.env` and a shareable `.env.example` are included; only the public API URL belongs in this frontend variable. In Render, set `FRONTEND_URL` to the exact deployed Vercel domain once it is available—this is required for production CORS.
